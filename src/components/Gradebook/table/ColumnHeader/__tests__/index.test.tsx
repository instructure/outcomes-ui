...
import { ColumnHeader, ColumnHeaderProps } from '../index'
...
describe('ColumnHeader', () => {
  ...
  it('renders children content', () => {
    renderInTable(<ColumnHeader {...defaultProps} />)
  ...
  it('renders as div when isStacked is true', () => {
    render(<ColumnHeader {...defaultProps} isStacked={true} />)
    const headerContent = screen.getByTestId('column-header-content')
  ...
  it('applies sticky position when isSticky is true', () => {
    renderInTable(<ColumnHeader {...defaultProps} isSticky={true} />)
    const header = screen.getByRole('columnheader')
  ...
  it('applies data-cell-id attribute', () => {
    renderInTable(<ColumnHeader {...defaultProps} data-cell-id="cell-0-0" />)
    const header = screen.getByRole('columnheader')
    expect(header).toHaveAttribute('data-cell-id', 'cell-0-0')
  })

  it('applies custom background when provided', () => {
    renderInTable(<ColumnHeader {...defaultProps} background="secondary" />)
    const header = screen.getByRole('columnheader')
    expect(header).toBeInTheDocument()
  })
  ...
  it('renders content wrapper with default opacity', () => {
    renderInTable(<ColumnHeader {...defaultProps} />)
    const contentWrapper = screen.getByTestId('column-header-content')
    expect(contentWrapper).toHaveStyle({opacity: 1})
  })

  it('applies reduced opacity when isDragging is true', () => {
    renderInTable(<ColumnHeader {...defaultProps} isDragging={true} />)
    const contentWrapper = screen.getByTestId('column-header-content')
  ...
  it('applies grab cursor when connectDragSource is provided', () => {
    ...
    renderInTable(<ColumnHeader {...defaultProps} connectDragSource={mockConnectDragSource} />)
    const contentWrapper = screen.getByTestId('column-header-content')
  ...
  it('connects drag preview with empty image when connectDragPreview is provided', () => {
    ...
    renderInTable(<ColumnHeader {...defaultProps} connectDragPreview={mockConnectDragPreview} />)
  ...
  it('filters out drag-drop specific props from DOM', () => {
    ...
    renderInTable(<ColumnHeader {...props} />)
    const header = screen.getByRole('columnheader')
