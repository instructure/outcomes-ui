...
describe('context/reducers', () => {
  ...
  describe('outcomes', () => {
    it('is merged with SET_OUTCOMES', () => {
      ...
      expect(newState.getIn(['outcomes', 'course_100']).toJS()).to.deep.equal(
    ...
    it('does not replace other context outcomes', () => {
      ...
      const newState = reduce(state, SET_OUTCOMES, newOutcomes)
      expect(Object.keys(newState.get('outcomes').toJS()).length).to.equal(2)
    })
    ...
    it('sets the scoring method on SET_SCORING_METHOD', () => {
      ...
      expect(newState.getIn(['outcomes', 'course_100', 10, 'scoring_method', 'points_possible'])).to.equal(5)
    })
  })
  ...
  describe('rootOutcomeIds', () => {
    it('is replaced by SET_ROOT_OUTCOME_IDS', () => {
      ...
      expect(newState.get('rootOutcomeIds').toJS()).to.deep.equal({ course_100: [4, 5, 6] })
    })
